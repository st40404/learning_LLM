...
# 第一階段訓練完成的模型路徑
first_finetuned_model_path = "./TinyLlama-finetune-hermes"

# 第二階段微調後的模型輸出路徑和 Hugging Face Repository ID
second_finetune_output_path = "./TinyLlama-continue-finetune-chatgpt-prompts"
second_huggingface_repo_id = "st40404/TinyLlama-continue-finetune-chatgpt-prompts"
...
def format_conversation(example):
    # fka/awesome-chatgpt-prompts 資料集結構為 'prompt' 和 'act'
    # 這裡將 'prompt' 作為使用者輸入
    # 因為沒有 assistant 的回答，所以我們訓練模型來生成回答
    # 格式化成 TinyLlama-Chat-V0.4 支援的格式
    
    # TinyLlama 使用 <|im_start|>role\n{content}<|im_end|>\n
